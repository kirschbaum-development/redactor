...
describe('Decoder', function () {
    it('unescapes a JSON line and reports the line span', function () {
    ...
    it('decodes a base64 token that holds text and skips words and binaries', function () {
        $secret = base64_encode('STRIPE=sk_live_4eC39HqLyjWDarjtT1zdp7dc');
        $binary = base64_encode(random_bytes(30));
        $window = "data:\n  key: {$secret}\n  blob: {$binary}\n  word: Authorization\n";

        $derived = Decoder::derive($window);
        $base64 = array_values(array_filter($derived, fn ($d) => $d->encoding === 'base64'));
    ...
    it('decodes a percent-encoded run', function () {
        $window = 'GET /cb?token=sk_live_4eC39HqLyjWDarjtT1zdp7dc%26redirect%3Dhttps%3A%2F%2Fu%3Ap%40h';

        $derived = Decoder::derive($window);
        $url = array_values(array_filter($derived, fn ($d) => $d->encoding === 'url'));

        expect($url)->not->toBeEmpty()
            ->and($url[0]->text)->toContain('https://u:p@h');
    });

    it('derives nothing from plain text', function () {
