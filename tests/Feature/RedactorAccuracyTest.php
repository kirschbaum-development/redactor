...
describe('Multibyte-correct entropy', function (): void {
    it('measures characters, not bytes', function (): void {
    ...
    it('agrees with the ASCII case for four distinct characters', function (): void {
        $strategy = new ShannonEntropyStrategy;
    ...
    it('reports zero for a single repeated multibyte character', function (): void {
        expect((new ShannonEntropyStrategy)->calculateShannonEntropy('日日日日日'))->toBe(0.0);
    });

    it('falls back to bytes for input that is not valid UTF-8', function (): void {
        $binary = "\xff\xfe\x00\x01\xff\xfe";

        expect((new ShannonEntropyStrategy)->calculateShannonEntropy($binary))->toBeGreaterThan(0.0);
    });

    it('counts min_length in characters', function (): void {
        ...
        expect(resolve(Redactor::class)->redact(['t' => '日本語能力試験合格者'], 'accuracy'))
            ->toBe(['t' => '日本語能力試験合格者']);
    });
});

describe('Per-charset entropy thresholds', function (): void {
    it('catches a hex digest that a base64-shaped threshold misses', function (): void {
        ...
        config()->set('redactor.profiles.accuracy', accuracyProfile());
        expect(resolve(Redactor::class)->redact(['h' => $digest], 'accuracy'))->toBe(['h' => $digest]);
        ...
        expect(resolve(Redactor::class)->redact(['h' => $digest], 'accuracy'))->toBe(['h' => '[REDACTED]']);
    });

    it('leaves the configured threshold in charge when no charset matches', function (): void {
        config()->set('redactor.profiles.accuracy', accuracyProfile([
            'shannon_entropy' => [
                'enabled' => true,
                'threshold' => 4.8,
    ...
    it('never silently overrides an explicitly configured threshold', function (): void {
        ...
        expect(resolve(Redactor::class)->redact(['h' => 'a94a8fe5ccb19ba61c4c0873d391e987982fbbd3'], 'accuracy'))
            ->toBe(['h' => '[REDACTED]']);
    });

    it('identifies the alphabets it claims to', function (): void {
...
describe('Capture-group aware replacement', function (): void {
    it('keeps the label and replaces only the secret', function (): void {
        ...
        expect(resolve(Redactor::class)->redact("aws_secret_access_key = {$secret}", 'capture'))
            ->toBe('aws_secret_access_key = [REDACTED]');
    });

    it('replaces the whole match when no capture group is declared', function (): void {
        config()->set('redactor.profiles.capture', accuracyProfile([
            'strategies' => [RegexPatternsStrategy::class],
            'shannon_entropy' => ['enabled' => false],
        ...
        expect(resolve(Redactor::class)->redact('aws_secret_access_key = wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY', 'capture'))
            ->toBe('[REDACTED]');
    });

    it('combines capture groups with partial mode', function (): void {
        config()->set('redactor.profiles.capture', accuracyProfile([
            'strategies' => [RegexPatternsStrategy::class],
            'shannon_entropy' => ['enabled' => false],
            'patterns' => [
        ...
        expect(resolve(Redactor::class)->redact('card: 4111111111111111 ok', 'capture'))
            ->toBe('card: ************1111 ok');
    });

    it('falls back to the whole match when the group did not participate', function (): void {
        config()->set('redactor.profiles.capture', accuracyProfile([
            'strategies' => [RegexPatternsStrategy::class],
            'shannon_entropy' => ['enabled' => false],
            'patterns' => [
        ...
        expect(resolve(Redactor::class)->redact('bare secret here', 'capture'))
            ->toBe('bare [REDACTED] here');
    });
});

describe('Shipped file_scan patterns', function (): void {
    it('no longer flags every 40-character alphanumeric run as an AWS key', function (): void {
    ...
    it('still catches an AWS secret key next to its label', function (): void {
    ...
    it('keeps the password label while removing the value', function (): void {
        $result = resolve(Redactor::class)->redact('DB_PASSWORD=sup3rs3cret', 'file_scan');

        expect($result)->not->toContain('sup3rs3cret')
            ->and(strtolower($result))->toContain('password');
    });

    it('keeps the host while removing url credentials', function (): void {
        $result = resolve(Redactor::class)->redact('https://admin:hunter2@db.example.com/x', 'file_scan');

        expect($result)->not->toContain('hunter2')
            ->and($result)->toContain('db.example.com');
    });

    it('still catches unambiguous single-token secrets outright', function (): void {
        foreach ([
            'AKIAIOSFODNN7EXAMPLE',
            'ghp_1234567890abcdefghijklmnopqrstuvwxyz',
            'sk_test_1234567890abcdef1234567890abcdef',
        ] as $secret) {
            expect(resolve(Redactor::class)->redact("value: {$secret}", 'file_scan'))
                ->not->toContain($secret);
        }
    });
});

describe('The ASCII tokenise path matches the Unicode one', function (): void {
    ...
    it('finds the same token in an ASCII value', function (): void {
        config()->set('redactor.profiles.tok', tokenProfile());

        expect(resolve(Redactor::class)->redact(['t' => 'key Zx7Qm4Kd9Rb2Vn6Tp end'], 'tok'))
            ->toBe(['t' => 'key [REDACTED] end']);
    });

    it('finds the same token when the value contains non-ASCII text', function (): void {
        config()->set('redactor.profiles.tok', tokenProfile());

        // Same secret, same neighbours, but the value is no longer ASCII - so
        // the /u pattern is used and must reach the same answer.
        expect(resolve(Redactor::class)->redact(['t' => 'клавиша Zx7Qm4Kd9Rb2Vn6Tp конец'], 'tok'))
            ->toBe(['t' => 'клавиша [REDACTED] конец']);
    });

    it('splits on a Unicode space, which the ASCII pattern would not', function (): void {
        config()->set('redactor.profiles.tok', tokenProfile());
        ...
        $result = resolve(Redactor::class)->redact(['t' => $value], 'tok');
    ...
    it('recognises ASCII and non-ASCII subjects correctly', function (): void {
        $strategy = new class extends ShannonEntropyStrategy
        {
