...
describe('Luhn', function (): void {
    it('accepts real card numbers', function (): void {
        foreach ([
    ...
    it('rejects numbers of the right shape that are not cards', function (): void {
        foreach ([
    ...
    it('rejects runs that are too short or too long to be a card', function (): void {
...
describe('IBAN', function (): void {
    it('accepts valid IBANs', function (): void {
        foreach ([
...
describe('SSN', function (): void {
    ...
    it('rejects the wrong number of digits', function (): void {
        expect(Validator::ssn('12345678'))->toBeFalse()
            ->and(Validator::ssn('1234567890'))->toBeFalse();
    });
});

describe('Validators inside redaction', function (): void {
    it('redacts a valid card and leaves an invalid lookalike alone', function (): void {
        ...
        expect(resolve(Redactor::class)->redact('card 4111111111111111 ok', 'validated'))
            ->toBe('card ************1111 ok');

        // An order number of the same shape survives.
        expect(resolve(Redactor::class)->redact('order 2024010112000001 ok', 'validated'))
            ->toBe('order 2024010112000001 ok');
    });

    it('does not mark a payload redacted when every match failed validation', function (): void {
        config()->set('redactor.profiles.validated', validatorProfile([
            'card' => ['pattern' => '/\b\d{16}\b/', 'validator' => 'luhn'],
        ]));

        $result = resolve(Redactor::class)->inspect(['n' => '1234567890123456'], 'validated');

        expect($result->wasRedacted)->toBeFalse()
            ->and($result->value)->toBe(['n' => '1234567890123456']);
    });

    it('redacts the valid matches and leaves the invalid ones in the same string', function (): void {
        config()->set('redactor.profiles.validated', validatorProfile([
            'card' => ['pattern' => '/\b\d{16}\b/', 'validator' => 'luhn'],
        ]));

        expect(resolve(Redactor::class)->redact('good 4111111111111111 bad 1234567890123456', 'validated'))
            ->toBe('good [REDACTED] bad 1234567890123456');
    });

    it('validates the capture group, not the surrounding context', function (): void {
        config()->set('redactor.profiles.validated', validatorProfile([
        ...
        expect(resolve(Redactor::class)->redact('card: 4111111111111111', 'validated'))
            ->toBe('card: [REDACTED]')
            ->and(resolve(Redactor::class)->redact('card: 1234567890123456', 'validated'))
            ->toBe('card: 1234567890123456');
    });

    it('applies validation in full mode too', function (): void {
        config()->set('redactor.profiles.validated', validatorProfile([
            'card' => ['pattern' => '/\b\d{16}\b/', 'validator' => 'luhn', 'mode' => 'full'],
        ]));

        expect(resolve(Redactor::class)->redact('n 1234567890123456', 'validated'))
            ->toBe('n 1234567890123456')
            ->and(resolve(Redactor::class)->redact('n 4111111111111111', 'validated'))
            ->toBe('[REDACTED]');
    });

    it('rejects an unknown validator name in config', function (): void {
        config()->set('redactor.profiles.validated', validatorProfile([
            'card' => ['pattern' => '/\d+/', 'validator' => 'vibes'],
        ]));

        expect(fn (): RedactorConfig => RedactorConfig::fromConfig('validated'))
            ->toThrow(\InvalidArgumentException::class, 'patterns.card.validator');
    });
});

describe('Shipped profiles use validators', function (): void {
    it('no longer flags an order number as a credit card', function (): void {
        $result = resolve(Redactor::class)->redact(['note' => 'order 2024010112000001 shipped'], 'default');

        expect($result['note'])->toBe('order 2024010112000001 shipped');
    });

    it('still redacts a real card in the default profile', function (): void {
        $result = resolve(Redactor::class)->redact(['note' => 'paid with 4111111111111111'], 'default');

        expect($result['note'])->not->toContain('4111111111111111')
            ->and($result['note'])->toContain('1111');
    });

    it('no longer flags 000-00-0000 as an SSN', function (): void {
        $result = resolve(Redactor::class)->redact(['note' => 'placeholder 000-00-0000'], 'default');

        expect($result['note'])->toBe('placeholder 000-00-0000');
    });

    it('redacts a valid IBAN', function (): void {
        $result = resolve(Redactor::class)->redact(['note' => 'pay GB82WEST12345698765432 now'], 'default');
