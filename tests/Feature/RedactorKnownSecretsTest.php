...
describe('Known secrets', function () {
    ...
    it('redacts a configured value wherever it appears verbatim', function () {
    ...
    it('is case-sensitive, because secrets are', function () {
        expect(app(Redactor::class)->redact('S3CR3T-VALUE-1', 'known'))->toBe('S3CR3T-VALUE-1');
    });

    it('reads secrets from config keys, including every string under an array', function () {
        ...
        expect(app(Redactor::class)->redact('acme-key-12345 / acme-secret-67890', 'known'))
            ->toBe('[REDACTED] / [REDACTED]');
    });

    it('refuses values too short to match safely', function () {
    ...
    it('accepts secrets registered at runtime, for every profile', function () {
    ...
    it('goes through the operator for its entity', function () {
        config()->set('redactor.profiles.known.operators', ['default' => 'redact', 'known_secret' => 'hash']);

        expect(app(Redactor::class)->redact('x s3cr3t-value-1 y', 'known'))
            ->toMatch('/^x \[known_secret:[a-z0-9]+\] y$/');
    });

    it('reports the finding as certain', function () {
        $result = app(Redactor::class)->redactWithMetadata('s3cr3t-value-1', 'known');

        expect($result->findings[0]->rule)->toBe('known_secret')
            ->and($result->findings[0]->confidence?->score)->toBe(1.0);
    });

    it('redacts APP_KEY in the shipped default profile', function () {
        $key = 'base64:'.base64_encode(random_bytes(32));
        config()->set('app.key', $key);

        $result = app(Redactor::class)->redact(['note' => "leaked {$key} here"]);

        expect($result['note'])->not->toContain($key)
            ->and($result['note'])->toStartWith('leaked ');
    });

    it('does not fail the profile when a configured secret is null', function () {
        config()->set('redactor.profiles.known.known_secrets', ['config' => ['services.nothing.key']]);

        expect(app(Redactor::class)->redact('fine', 'known'))->toBe('fine');
    });
});
