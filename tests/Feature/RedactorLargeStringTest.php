...
describe('Long strings', function () {
    ...
    it('keeps the head of a long string and notes what was cut', function () {
        $value = str_repeat('trace line. ', 10); // 120 bytes

        $result = app(Redactor::class)->redactWithMetadata($value, 'long');
    ...
    it('still scans the head it keeps', function () {
        $value = 'contact bob@example.com about '.str_repeat('x', 100);

        $result = app(Redactor::class)->redact($value, 'long');

        expect($result)->toStartWith('contact [REDACTED] about ')
            ->and($result)->not->toContain('bob@example.com');
    });

    it('never splits a multibyte character at the cut', function () {
    ...
    it('replaces the whole value when the behaviour is redact', function () {
        config()->set('redactor.profiles.long.large_string_behavior', 'redact');

        $result = app(Redactor::class)->redact(str_repeat('a', 100), 'long');

        expect($result)->toBe('[REDACTED] (String with 100 characters)');
    });

    it('rejects an unknown behaviour', function () {
        config()->set('redactor.profiles.long.large_string_behavior', 'shrug');

        app(Redactor::class)->redact('x', 'long');
    })->throws(\InvalidArgumentException::class, 'large_string_behavior');

    it('leaves strings at or under the limit alone', function () {
        $value = str_repeat('a', 40);

        expect(app(Redactor::class)->redact($value, 'long'))->toBe($value);
    });
});
