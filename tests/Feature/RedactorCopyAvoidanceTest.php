...
describe('Returning the input when nothing changed', function (): void {
    beforeEach(fn () => config()->set('redactor.profiles.copy', copyProfile()));

    it('returns a clean payload exactly as it arrived', function (): void {
        $payload = [
            'level' => 'info',
            'nested' => ['a' => 1, 'b' => ['c' => 'text']],
            'list' => [1, 2, 3],
        ...
        expect(resolve(Redactor::class)->redact($payload, 'copy'))->toBe($payload);
    });

    it('preserves key order and key types', function (): void {
        $payload = ['z' => 1, 'a' => 2, 3 => 'three', 'm' => 4];

        $result = resolve(Redactor::class)->redact($payload, 'copy');
    ...
    it('still redacts, and only what it should', function (): void {
    ...
    it('leaves untouched siblings alone when one branch changes', function (): void {
        $payload = [
    ...
    it('preserves order when a key is removed', function (): void {
        config()->set('redactor.profiles.copy', copyProfile([
            'paths' => ['b' => 'remove'],
        ]));

        $result = resolve(Redactor::class)->redact(['a' => 1, 'b' => 2, 'c' => 3], 'copy');

        expect($result)->toBe(['a' => 1, 'c' => 3])
            ->and(array_keys($result))->toBe(['a', 'c']);
    });

    it('removes a list entry without renumbering the rest', function (): void {
        config()->set('redactor.profiles.copy', copyProfile([
            'paths' => ['1' => 'remove'],
        ]));

        $result = resolve(Redactor::class)->redact(['zero', 'one', 'two'], 'copy');

        expect($result)->toBe([0 => 'zero', 2 => 'two']);
    });

    it('does not report a redaction for an untouched payload', function (): void {
        $result = resolve(Redactor::class)->inspect(['a' => 'clean', 'b' => ['c' => 'also clean']], 'copy');

        expect($result->wasRedacted)->toBeFalse()
            ->and($result->findings)->toBe([]);
    });

    it('handles an empty array and an empty nested array', function (): void {
        expect(resolve(Redactor::class)->redact([], 'copy'))->toBe([])
            ->and(resolve(Redactor::class)->redact(['a' => []], 'copy'))->toBe(['a' => []]);
    });

    it('does not mutate the array it was given', function (): void {
        $payload = ['password' => 'hunter2', 'keep' => 'ordinary'];
        $before = $payload;

        resolve(Redactor::class)->redact($payload, 'copy');
