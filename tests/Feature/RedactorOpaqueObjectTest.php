...
describe('Opaque objects', function () {
    it('passes a Throwable through untouched so the formatter still renders the trace', function () {
        $exception = new \RuntimeException('db down');

        $result = (new RedactorProcessor(app(Redactor::class)))(opaqueRecord(['exception' => $exception]));
    ...
    it('passes dates, enums and closures through untouched', function () {
        $when = Carbon::parse('2026-09-13 10:00:00');
        $closure = fn () => 1;
    ...
    it('still lets a key rule win over an opaque value', function () {
        $result = app(Redactor::class)->redact([
    ...
    it('preserves an opaque object nested inside a structure that is otherwise redacted', function () {
    ...
    it('raises no deprecation while walking objects', function () {
        ...
        try {
            ...
            $result = app(Redactor::class)->redact(['payload' => $object, 'other' => new \ArrayObject(['secret' => 'x'])]);
